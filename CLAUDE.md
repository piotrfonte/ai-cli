...
5. **Model pinning**: launches `opencode -m mlx/<model-id>`, which **overrides** `opencode.json`'s default `model` (a Vercel AI Gateway model — see below). So `ai` / `-l` / `-g` always run their local oMLX model regardless of what a bare `opencode` defaults to.
